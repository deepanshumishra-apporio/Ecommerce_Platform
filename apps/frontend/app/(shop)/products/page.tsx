...
export default function ShopProductsPage() {
    const { filters, categories, loading, error, setFilters, fetchProducts, fetchCategories, getProducts } = useProductStore();
    const products = getProducts();

    useEffect(() => { fetchCategories(); }, [fetchCategories]);
    useEffect(() => { fetchProducts(); }, [filters, fetchProducts]);
    ...
    return (
        <div className="min-h-screen bg-white">
                ...
                <div className="max-w-6xl mx-auto flex items-end justify-between flex-wrap gap-2">
                    ...
                    {products && (
                        <p className="text-zinc-400 text-sm">
                            ...
                            {filters.search ? <> for <span className="text-white font-black">"{filters.search}"</span></> : ""}
                        </p>
                    )}
                </div>
            </div>
            ...
            <div className="max-w-6xl mx-auto px-6 py-8">
                ...
                {/* Filter bar */}
                <div className="flex flex-wrap gap-2 mb-8 p-4 bg-zinc-50 border border-zinc-100">
                    <input
                    ...
                    {(filters.search || filters.categoryId || filters.sort !== "createdAt_desc") && (
                        <button
                            onClick={() => setFilters({ search: "", categoryId: "", sort: "createdAt_desc", page: 1 })}
                            className="border border-zinc-200 px-5 py-2.5 text-xs font-black uppercase tracking-widest hover:border-black transition-colors flex items-center gap-1.5"
                        >
                    ...
                    <div className="flex flex-wrap gap-2 mb-6">
                        {filters.search && (
                            <span className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest bg-black text-white px-2.5 py-1">
                                ...
                                <button onClick={() => setFilters({ search: "" })} className="hover:text-zinc-300 transition-colors">×</button>
                            </span>
                        )}
                        {filters.categoryId && (
                            <span className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest bg-zinc-100 text-zinc-700 px-2.5 py-1">
                                ...
                                <button onClick={() => setFilters({ categoryId: "" })} className="hover:text-red-500 transition-colors">×</button>
                            </span>
                        )}
                    </div>
                )}
                ...
                {error && (
                    <div className="border-l-4 border-red-500 bg-red-50 text-red-600 px-4 py-3 text-sm mb-8">{error}</div>
                )}
                    ...
                    <>
                        {products.items.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-28 gap-4">
                                ...
                                <button onClick={() => setFilters({ search: "", categoryId: "", sort: "createdAt_desc", page: 1 })} className="border border-zinc-300 px-6 py-2 text-xs font-black uppercase tracking-widest hover:border-black transition-colors">
                                    Clear Filters
                                </button>
                            </div>
                        ) : (
                                ...
                                {products.items.map((p: Product) => (
                                    <div key={p.id} className="group border border-zinc-100 bg-white hover:border-zinc-300 hover:shadow-md transition-all duration-200 overflow-hidden flex flex-col">
                                        <a href={`/products/${p.id}`} className="block relative overflow-hidden bg-zinc-50" style={{ aspectRatio: "1" }}>
                                            {p.featuredImage ? (
                                                <img src={p.featuredImage} alt={p.name} loading="lazy" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-[1.07]" />
                                            ) : (
                                            ...
                                            {p.stock > 0 && p.stock <= 5 && (
                                                <span className="absolute top-2.5 left-2.5 bg-amber-500 text-white text-[8px] font-black px-2 py-0.5 uppercase tracking-widest z-10">Only {p.stock} left</span>
                                            )}
                                        ...
                                        <div className="p-3 flex flex-col gap-1 flex-1">
                                            ...
                                            <a href={`/products/${p.id}`} className="font-black text-sm uppercase truncate block leading-tight hover:underline underline-offset-2 transition-colors">{p.name}</a>
                        ...
                        {products.meta.totalPages > 1 && (
                            <div className="flex items-center justify-center gap-3 mt-12">
                                {filters.page > 1 ? (
                                    <button onClick={() => setFilters({ page: filters.page - 1 })} className="flex items-center gap-2 border border-zinc-200 px-5 py-2.5 text-xs font-black uppercase tracking-widest hover:border-black hover:bg-black hover:text-white transition-all duration-200 group">
                                ...
                                <div className="flex items-center gap-1">
                                    {Array.from({ length: Math.min(products.meta.totalPages, 5) }, (_, i) => {
                                        const pg = i + 1;
                                        return (
                                            <button key={pg} onClick={() => setFilters({ page: pg })}
                                                className={`w-9 h-9 flex items-center justify-center text-xs font-black transition-all duration-200 ${pg === filters.page ? "bg-black text-white" : "border border-zinc-200 hover:border-black"}`}>
                                                {pg}
                                            </button>
                                        );
                                    })}
