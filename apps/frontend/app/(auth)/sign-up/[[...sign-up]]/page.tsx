...
export default function SignUpPage() {
  ...
  const strengthColor = ["", "#FF3131", "#F59E0B", "#00FF94"][strength];
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-6 py-12">
          <div className="w-full max-w-md">
            ...
            <div className="hidden lg:block mb-8">
              <p className="text-[10px] font-black tracking-[0.45em] uppercase text-[#00FF94] mb-2">Join The Tribe</p>
              <h1 className="text-3xl font-black uppercase tracking-tight">Create Account</h1>
              <p className="text-zinc-400 text-sm mt-2">Sign up for free. No credit card required.</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="bg-white border border-zinc-200 w-full p-8 flex flex-col gap-5">
                ...
                {password.length > 0 && (
                  <div className="flex items-center gap-2 mt-0.5">
                    <div className="flex gap-1 flex-1">
                      {[1, 2, 3].map(i => (
                        <div key={i} className="h-1 flex-1 rounded-full transition-all duration-300" style={{ background: i <= strength ? strengthColor : "#e5e7eb" }} />
                      ))}
                    </div>
                    <span className="text-[10px] font-black uppercase tracking-widest" style={{ color: strengthColor }}>{strengthLabel}</span>
                  </div>
                )}
              </div>
              ...
              {/* Confirm Password */}
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-black tracking-[0.25em] uppercase text-zinc-400">Confirm Password</label>
                <div className="relative">
                  <input
                    ...
                    required
                    autoComplete="new-password"
                    className={`w-full bg-zinc-50 border text-black text-sm px-3 py-2.5 pr-10 outline-none focus:border-black transition-colors placeholder:text-zinc-300 ${
                      confirm && confirm !== password ? "border-red-300" : "border-zinc-200"
                    }`}
                  />
                  <button
                    type="button"
                ...
                {confirm && confirm !== password && (
                  <span className="text-[10px] font-black text-[#FF3131] uppercase tracking-widest">Passwords do not match</span>
                )}
                {confirm && confirm === password && (
                  <span className="text-[10px] font-black text-[#00FF94] uppercase tracking-widest">✓ Passwords match</span>
                )}
              </div>
              ...
              <p className="text-[10px] text-zinc-400 text-center leading-relaxed">
                ...
                <span className="text-black font-black cursor-pointer hover:underline">Terms</span> &{" "}
                <span className="text-black font-black cursor-pointer hover:underline">Privacy Policy</span>.
              </p>
            </form>
